...
import ewe.diagram.edit.commands.LoadXMLModelTaskCreateCommand;
import ewe.diagram.edit.commands.NestedElementCreateCommand;
import ewe.diagram.edit.commands.PropertyCreateCommand;
...
public class TargetTargetTargetTasksCompartmentItemSemanticEditPolicy extends
	...
	protected Command getCreateCommand(CreateElementRequest req) {
		...
		if (EweElementTypes.LoadXMLModelTask_3007 == req.getElementType()) {
			return getGEFWrapper(new LoadXMLModelTaskCreateCommand(req));
		}
		if (EweElementTypes.FlockTask_3008 == req.getElementType()) {
			return getGEFWrapper(new FlockTaskCreateCommand(req));
		}
		if (EweElementTypes.EUnitTask_3009 == req.getElementType()) {
			return getGEFWrapper(new EUnitTaskCreateCommand(req));
		}
		if (EweElementTypes.EclTask_3010 == req.getElementType()) {
			return getGEFWrapper(new EclTaskCreateCommand(req));
		}
		if (EweElementTypes.LoadModelTask_3011 == req.getElementType()) {
			return getGEFWrapper(new LoadModelTaskCreateCommand(req));
		}
		if (EweElementTypes.StoreModelTask_3012 == req.getElementType()) {
			return getGEFWrapper(new StoreModelTaskCreateCommand(req));
		}
		if (EweElementTypes.RollbackTransactionTask_3013 == req
				.getElementType()) {
			return getGEFWrapper(new RollbackTransactionTaskCreateCommand(req));
		}
		if (EweElementTypes.DisposeModelTask_3014 == req.getElementType()) {
			return getGEFWrapper(new DisposeModelTaskCreateCommand(req));
		}
		if (EweElementTypes.StartTransactionTask_3015 == req.getElementType()) {
			return getGEFWrapper(new StartTransactionTaskCreateCommand(req));
		}
		if (EweElementTypes.CommitTransactionTask_3016 == req.getElementType()) {
			return getGEFWrapper(new CommitTransactionTaskCreateCommand(req));
		}
		if (EweElementTypes.Property_3017 == req.getElementType()) {
			return getGEFWrapper(new PropertyCreateCommand(req));
		}
		if (EweElementTypes.NestedElement_3018 == req.getElementType()) {
			return getGEFWrapper(new NestedElementCreateCommand(req));
		}
		if (EweElementTypes.Task_3019 == req.getElementType()) {
