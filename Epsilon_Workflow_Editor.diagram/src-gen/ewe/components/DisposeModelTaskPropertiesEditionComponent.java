...
public class DisposeModelTaskPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (disposeModelTask.getModel() != null && isAccessible(EweViewsRepository.DisposeModelTask.Properties.model))
				basePart.setModel(EEFConverterUtil.convertToString(EcorePackage.eINSTANCE.getEString(), disposeModelTask.getModel()));
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == EweViewsRepository.DisposeModelTask.Properties.model) {
			return EwePackage.eINSTANCE.getDisposeModelTask_Model();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (EweViewsRepository.DisposeModelTask.Properties.model == event.getAffectedEditor()) {
			disposeModelTask.setModel((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.eINSTANCE.getEString(), (String)event.getNewValue()));
		}
	}
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (EwePackage.eINSTANCE.getDisposeModelTask_Model().equals(msg.getFeature()) && basePart != null && isAccessible(EweViewsRepository.DisposeModelTask.Properties.model)) {
				if (msg.getNewValue() != null) {
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (EweViewsRepository.DisposeModelTask.Properties.model == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(EwePackage.eINSTANCE.getDisposeModelTask_Model().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(EwePackage.eINSTANCE.getDisposeModelTask_Model().getEAttributeType(), newValue);
				}
