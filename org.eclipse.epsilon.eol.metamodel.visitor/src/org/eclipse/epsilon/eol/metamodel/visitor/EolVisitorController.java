...
public class EolVisitorController<T, R> {
	...
	protected List<IModelVisitor<T, R>> iModelVisitors = new ArrayList<IModelVisitor<T, R>>();
	...
	public R visit(EObject o, T context) {
		...
		if (o instanceof SelfInnermostContentType && !selfInnermostContentTypeVisitors.isEmpty()) {
		...
		if (o instanceof IModel && !iModelVisitors.isEmpty()) {
			for (IModelVisitor<T, R> visitor : iModelVisitors) {
				if (visitor.appliesTo((IModel) o, context)) {
					return visitor.visit((IModel) o, context, this);
				}
			}
		}
	...
	public R visitAsSelfInnermostContentType(SelfInnermostContentType selfInnermostContentType, T context) {
		if (!selfInnermostContentTypeVisitors.isEmpty()) {
			for (SelfInnermostContentTypeVisitor<T, R> visitor : selfInnermostContentTypeVisitors) {
	...
	public R visitAsIModel(IModel iModel, T context) {
		if (!iModelVisitors.isEmpty()) {
			for (IModelVisitor<T, R> visitor : iModelVisitors) {
				if (visitor.appliesTo((IModel) iModel, context)) {
					return visitor.visit((IModel) iModel, context, this);
				}
			}
		}
		return null;
	...
	public boolean addSelfInnermostContentTypeVisitor(SelfInnermostContentTypeVisitor<T, R> selfInnermostContentTypeVisitor) {
	...
	public boolean addIModelVisitor(IModelVisitor<T, R> iModelVisitor) {
		return this.iModelVisitors.add(iModelVisitor);
	}
	...
	public boolean removeIModelVisitor(IModelVisitor<T, R> iModelVisitor) {
		return this.iModelVisitors.remove(iModelVisitor);
	}
