...
public class EolVisitorController<T, R> {
	...
	protected List<DomElementVisitor<T, R>> domElementVisitors = new ArrayList<DomElementVisitor<T, R>>();
	...
	public R visit(EObject o, T context) {
		...
		if (o instanceof DomElement && !domElementVisitors.isEmpty()) {
			for (DomElementVisitor<T, R> visitor : domElementVisitors) {
				if (visitor.appliesTo((DomElement) o, context)) {
					return visitor.visit((DomElement) o, context, this);
				}
			}
		}
	...
	public R visitAsDomElement(DomElement domElement, T context) {
		if (!domElementVisitors.isEmpty()) {
			for (DomElementVisitor<T, R> visitor : domElementVisitors) {
				if (visitor.appliesTo((DomElement) domElement, context)) {
					return visitor.visit((DomElement) domElement, context, this);
				}
			}
		}
		return null;
	...
	public boolean addDomElementVisitor(DomElementVisitor<T, R> domElementVisitor) {
		return this.domElementVisitors.add(domElementVisitor);
	}
	...
	public boolean removeDomElementVisitor(DomElementVisitor<T, R> domElementVisitor) {
		return this.domElementVisitors.remove(domElementVisitor);
	}
