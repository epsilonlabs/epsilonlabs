...
public class SwitchCaseExpressionCreatorTest {
	...
	public void test() {
		...
		assertEquals(switchStatement.getCases().size(), 2);
		
		
		SwitchCaseExpressionStatement statement1 = switchStatement.getCases().get(0);
		ExpressionOrStatementBlock case1Block = statement1.getBody();
		assertEquals(case1Block.getExpression(), null);
		assertEquals(case1Block.getBlock().getStatements().size(), 1);
		assertEquals(case1Block.getBlock().getStatements().get(0).getClass(), ExpressionStatementImpl.class);
		ExpressionStatement expressionStatement2 = (ExpressionStatement) case1Block.getBlock().getStatements().get(0);
		
		assertEquals(expressionStatement2.getExpression().getClass(), MethodCallExpressionImpl.class);
		MethodCallExpression methodCallExpression2 = (MethodCallExpression) expressionStatement2.getExpression();
		assertEquals(methodCallExpression2.getTarget().getClass(), IntegerExpressionImpl.class);
		IntegerExpression integerExpression = (IntegerExpression) methodCallExpression2.getTarget();
		assertEquals(integerExpression.getValue(), 1);
		assertEquals(methodCallExpression2.getMethod().getName(), "println");
		assertEquals(methodCallExpression2.getArguments().size(), 0);
