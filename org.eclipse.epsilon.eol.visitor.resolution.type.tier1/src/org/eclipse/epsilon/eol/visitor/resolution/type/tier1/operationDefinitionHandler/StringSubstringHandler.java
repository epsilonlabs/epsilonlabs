...
public class StringSubstringHandler extends StringOperationDefinitionHandler{

	@Override
	public boolean appliesTo(String name, Type contextType,
			ArrayList<Type> argTypes) {
		boolean result = true;
		if ((name.equals("substring") && argTypes.size() == 1) ||
				(name.equals("substring") && argTypes.size() == 2)) {
	...
	public OperationDefinition handle(
		...
		if (result != null) {
			...
			if (argTypes.size() == 1) {
				...
				if (argType2 instanceof IntegerType) {
				}
				else if (TypeUtil.getInstance().isInstanceofAnyType(argType2)) {
					if (TypeInferenceManager.getInstance().containsDynamicType((AnyType) argType2, EolPackage.eINSTANCE.getIntegerType())) {
					}
					else {
						LogBook.getInstance().addError(argType2, IMessage_TypeResolution.EXPRESSION_MAY_NOT_BE_INTEGER);
					}
				}
				else {
					LogBook.getInstance().addError(argType2, IMessage_TypeResolution.EXPRESSION_MAY_NOT_BE_INTEGER);
