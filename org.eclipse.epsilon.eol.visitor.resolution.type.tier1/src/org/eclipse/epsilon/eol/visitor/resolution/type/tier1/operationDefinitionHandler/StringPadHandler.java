...
public class StringPadHandler extends StringOperationDefinitionHandler {

	@Override
	public boolean appliesTo(String name, Type contextType,
			ArrayList<Type> argTypes) {
		if (name.equals("pad") && argTypes.size() == 3) {
	...
	public OperationDefinition handle(
		...
		if (result != null) {
			...
			Type argType1 = argTypes.get(0);
			Type argType2 = argTypes.get(1);
			Type argType3 = argTypes.get(2);
			...
			else if (TypeUtil.getInstance().isInstanceofAnyType(argType1)) {
			...
			if (argType2 instanceof StringType) {
			}
			else if (TypeUtil.getInstance().isInstanceofAnyType(argType2)) {
				if (TypeInferenceManager.getInstance().containsDynamicType((AnyType) argType2, EolPackage.eINSTANCE.getStringType())) {
				}
				else {
					LogBook.getInstance().addError(argType2, IMessage_TypeResolution.EXPRESSION_MAY_NOT_BE_STRING);
				}
			}
			else {
				LogBook.getInstance().addError(argType2, IMessage_TypeResolution.EXPRESSION_MAY_NOT_BE_STRING);
			...
			if (argType3 instanceof BooleanType) {
			}
			else if (TypeUtil.getInstance().isInstanceofAnyType(argType3)) {
				if (TypeInferenceManager.getInstance().containsDynamicType((AnyType) argType3, EolPackage.eINSTANCE.getBooleanType())) {
				}
				else {
					LogBook.getInstance().addError(argType3, IMessage_TypeResolution.EXPRESSION_MAY_NOT_BE_BOOLEAN);
				}
			}
			else {
				LogBook.getInstance().addError(argType3, IMessage_TypeResolution.EXPRESSION_MAY_NOT_BE_BOOLEAN);
