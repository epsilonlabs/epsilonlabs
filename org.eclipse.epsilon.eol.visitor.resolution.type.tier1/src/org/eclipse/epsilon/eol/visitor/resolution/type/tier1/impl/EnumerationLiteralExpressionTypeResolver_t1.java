...
public class EnumerationLiteralExpressionTypeResolver_t1 extends EnumerationLiteralExpressionVisitor<TypeResolutionContext, Object>{
	...
	public Object visit(
		...
		if (context.getTypeUtil().getIMetamodelDriverByName(modelName).size() == 0) {
			context.getLogBook().addError(enumerationLiteralExpression, IMessage_TypeResolution.bindMessage(IMessage_TypeResolution.MODEL_NOT_FOUND, modelName));
		}
		else {
				...
				if (iPackageDrivers.size() == 0) {
					...
					if (iPackageDriver.containsEnumeration(enumName)) {
						...
						if (enum1.getEEnumLiteral(literalName) != null) {
							context.getLogBook().addError(enumerationLiteralExpression, IMessage_TypeResolution.ENUM_LITERAL_CANNOT_BE_RESLVED);
						}
						else {
							return null;
						}
					...
					context.getLogBook().addError(enumerationLiteralExpression, IMessage_TypeResolution.ENUM_DUPLICATE);
				}
			}
		}
