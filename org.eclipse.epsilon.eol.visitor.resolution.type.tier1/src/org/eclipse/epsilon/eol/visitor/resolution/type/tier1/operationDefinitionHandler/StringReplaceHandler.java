...
public class StringReplaceHandler extends StringOperationDefinitionHandler {

	@Override
	public boolean appliesTo(String name, Type contextType,
			ArrayList<Type> argTypes) {
		boolean result = true;
		if (name.equals("replace") && argTypes.size() == 2) {
	...
	public OperationDefinition handle(
		...
		if (result != null) {
			...
			if (argType1 instanceof StringType) {
			}
			else if (TypeUtil.getInstance().isInstanceofAnyType(argType1)) {
				if (TypeInferenceManager.getInstance().containsDynamicType((AnyType) argType1, EolPackage.eINSTANCE.getStringType())) {
				}
				else {
					LogBook.getInstance().addError(argType1, IMessage_TypeResolution.EXPRESSION_MAY_NOT_BE_STRING);
				}
			}
			else {
				LogBook.getInstance().addError(argType1, IMessage_TypeResolution.EXPRESSION_MAY_NOT_BE_STRING);
