...
public class TransformationRuleCreator extends EtlElementCreator{
	...
	public EOLElement create(AST ast, EOLElement container,
		...
		if (nameAST != null) {
			rule.setName((NameExpression) _context.getEtlElementCreatorFactory().createDomElement(nameAST, rule, _context));
		}
		AST fromAst = nameAST.getNextSibling();
		if (fromAst.getType() == EtlParser.FORMAL) {
			rule.setSource((FormalParameterExpression) _context.getEtlElementCreatorFactory().createDomElement(fromAst, rule, _context));
		}
		...
		if (toAsts!=null && toAsts.getType() == EtlParser.PARAMLIST) {
			...
			{
				if (toAst != null) {
					rule.getTargets().add((FormalParameterExpression) _context.getEtlElementCreatorFactory().createDomElement(toAst, rule, _context));
				}
			}
		}
		...
		if (extendsAsts != null) {
			...
			{
				rule.getExtends().add((NameExpression) _context.getEtlElementCreatorFactory().createDomElement(extendsAst, rule, _context));
			}
		}
		
		AST blockAst = AstUtilities.getChild(ast, EtlParser.BLOCK);
		if (blockAst!=null) {
			rule.setBody((Block) _context.getEtlElementCreatorFactory().createDomElement(blockAst, rule, _context));
		}
		
		AST guardAst = AstUtilities.getChild(ast, EtlParser.GUARD);
		if (guardAst != null && guardAst.getFirstChild() != null) {
			rule.setGuard((ExpressionOrStatementBlock) _context.getEtlElementCreatorFactory().createDomElement(guardAst.getFirstChild(), rule, _context, ExpressionOrStatementBlockCreator.class));
		}
