...
public class NamedBlockCreator extends EtlElementCreator{
	...
	public EOLElement create(AST ast, EOLElement container,
		...
		if (firstChild!=null & firstChild.getType() != EtlParser.BLOCK) {
			namedBlock.setName((NameExpression) _context.getEtlElementCreatorFactory().createDomElement(firstChild, namedBlock, _context));
		}
		
		AST blockAst = AstUtilities.getChild(ast, EtlParser.BLOCK);
		if (blockAst != null) {
			namedBlock.setBody((Block) _context.getEtlElementCreatorFactory().createDomElement(blockAst, namedBlock, _context));
		}
