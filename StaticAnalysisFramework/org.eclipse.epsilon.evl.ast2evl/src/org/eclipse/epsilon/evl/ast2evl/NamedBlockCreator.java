...
public class NamedBlockCreator extends EvlElementCreator{
	...
	public EOLElement create(AST ast, EOLElement container,
		...
		if (firstChild!=null & firstChild.getType() != EvlParser.BLOCK) {
			namedBlock.setName((NameExpression) _context.getEvlElementCreatorFactory().createDomElement(firstChild, namedBlock, _context));
		}
		
		AST blockAst = AstUtilities.getChild(ast, EvlParser.BLOCK);
		if (blockAst != null) {
			namedBlock.setBody((Block) _context.getEvlElementCreatorFactory().createDomElement(blockAst, namedBlock, _context));
		}
